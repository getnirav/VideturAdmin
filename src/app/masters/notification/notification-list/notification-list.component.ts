...
export class NotificationListComponent implements OnInit, AfterViewInit {
  ...
  public displayedColumns = ['select', 'eventCode', 'notificationType', 'tone', 'appFrequency',
    'appFrequencyInterval', 'appContent', 'emailFrequency', 'emailFrequencyInterval',
    'emailSubject', 'emailContent', 'action'];
  ...
  constructor(private http: HttpClient, public modalService: NgbModal,
    private notificationService: NotificationService, private alertify: AlertifyService,
    ...
    this.notificationService.getAll().subscribe(
      result => {
        ...
        if (result.isSuccess) {
          ...
          this.dataSource.data = result.list as Notification[];
        } else {
  ...
  openAddPopup(link: any) {
    ...
    // console.log(link);
    //const modalRef = this.modalService.open(NotificationEntryComponent);
    const modalRef = this.modalService.open(NotificationEntryComponent, { size: 'xl' });
  ...
  open(content: any, autoId: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      if (result === 'yes') {
        this.notificationService.delete(autoId).subscribe(
          result => {
            // console.log(Countries);
            if (result.isSuccess) {
  ...
  DeleteSelection(content: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      if (result === 'yes') {
        ...
        if (autoIds.length > 0) {
          this.notificationService.deleteMultiple(autoIds).subscribe(
            result => {
              if (result.isSuccess) {
                this.alertify.success(result.message);
              } else {
  ...
  ExportData(fileType: string) {
    ...
    if (autoIds.length != -1) {
      let ModuleName: string = 'Notification'
      if (fileType == "pdf") {
