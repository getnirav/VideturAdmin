...
import { AppearanceTypeEntryComponent } from '../appearance-type-entry/appearance-type-entry.component';

@Component({
  selector: 'app-appearance-type-list',
  templateUrl: './appearance-type-list.component.html',
  styleUrls: ['./appearance-type-list.component.css']
})
export class AppearanceTypeListComponent implements OnInit, AfterViewInit {

  public displayedColumns = ['select', 'practiceArea', 'appearanceType1', 'description', 'action'];
  public dataSource = new MatTableDataSource<AppearanceType>();
  @ViewChild(MatSort) sort!: MatSort
  @ViewChild(MatPaginator) paginator!: MatPaginator;
  selection = new SelectionModel<AppearanceType>(true, []);
  ...
  constructor(private http: HttpClient, public modalService: NgbModal,
    private appearanceTypeService: AppearanceTypeService, private alertify: AlertifyService,
  ...
  fillData() {
    this.appearanceTypeService.getAll().subscribe(result => {
      // console.log(Countries);
      if (result.isSuccess) {
        // this.AppearanceTypes = result.list;
        this.dataSource.data = result.list as AppearanceType[];
      } else {
  ...
  openAddPopup(link: any) {
    ...
    // console.log(link);
    const modalRef = this.modalService.open(AppearanceTypeEntryComponent);
  ...
  open(content: any, autoId: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      if (result === 'yes') {
        this.appearanceTypeService.delete(autoId).subscribe(
          result => {
            // console.log(Countries);
            if (result.isSuccess) {
  ...
  DeleteSelection(content: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      if (result === 'yes') {
        ...
        if (autoIds.length > 0) {
          this.appearanceTypeService.deleteMultiple(autoIds).subscribe(
            result => {
              if (result.isSuccess) {
                this.alertify.success(result.message);
              } else {
  ...
  ExportData(fileType: string) {
    ...
    if (autoIds.length != -1) {
      let ModuleName: string = 'Appearance Type'
      if (fileType == "pdf") {
