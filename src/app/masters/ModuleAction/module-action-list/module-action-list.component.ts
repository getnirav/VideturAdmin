...
import { ModuleActionEntryComponent } from '../module-action-entry/module-action-entry.component';

@Component({
  selector: 'app-module-action-list',
  templateUrl: './module-action-list.component.html',
  styleUrls: ['./module-action-list.component.css']
})
export class ModuleActionListComponent implements OnInit, AfterViewInit {

  public displayedColumns = ['select', 'moduleName', 'actionName', 'action'];
  public dataSource = new MatTableDataSource<ModuleAction>();
  @ViewChild(MatSort) sort!: MatSort
  @ViewChild(MatPaginator) paginator!: MatPaginator;
  selection = new SelectionModel<ModuleAction>(true, []);
  ...
  constructor(private http: HttpClient, public modalService: NgbModal,
    private moduleActionService: ModuleActionService, private alertify: AlertifyService,
  ...
  ngOnInit() {
    this.moduleActionService.getAll().subscribe(result => {
      ...
      if (result.isSuccess) {
        ...
        this.dataSource.data = result.list as ModuleAction[];
      } else {
  ...
  openAddPopup(link: any) {
    ...
    // console.log(link);
    const modalRef = this.modalService.open(ModuleActionEntryComponent);
  ...
  open(content: any, autoId: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      if (result === 'yes') {
        this.moduleActionService.delete(autoId).subscribe(
          result => {
            // console.log(Countries);
            if (result.isSuccess) {
  ...
  DeleteSelection(content: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      if (result === 'yes') {
        ...
        if (autoIds.length > 0) {
          this.moduleActionService.deleteMultiple(autoIds).subscribe(
            result => {
              if (result.isSuccess) {
                this.alertify.success(result.message);
              } else {
  ...
  ExportData(fileType: string) {
    ...
    if (autoIds.length != -1) {
      let ModuleName: string = 'Module Action'
      if (fileType == "pdf") {
