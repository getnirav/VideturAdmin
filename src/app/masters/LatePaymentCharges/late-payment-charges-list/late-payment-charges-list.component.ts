...
import { LatePaymentChargesEntryComponent } from '../late-payment-charges-entry/late-payment-charges-entry.component';

@Component({
  selector: 'app-late-payment-charges-list',
  templateUrl: './late-payment-charges-list.component.html',
  styleUrls: ['./late-payment-charges-list.component.css']
})
export class LatePaymentChargesListComponent implements OnInit {


  public displayedColumns = ['select', 'fromDays', 'toDays', 'penaltyPercent', 'description', 'action'];
  public dataSource = new MatTableDataSource<LatePaymentCharges>();
  @ViewChild(MatSort) sort!: MatSort
  @ViewChild(MatPaginator) paginator!: MatPaginator;

  selection = new SelectionModel<LatePaymentCharges>(true, []);
  ...
  constructor(private http: HttpClient, public modalService: NgbModal,
    private latePaymentChargesService: LatePaymentChargesService, private alertify: AlertifyService,
    private datePipe: DatePipe, private importExportDataService: ImportExportDataService) { }

  ngOnInit(): void {
    this.latePaymentChargesService.getAll().subscribe(result => {
      // console.log(Countries);
      if (result.isSuccess) {
        // this.Courts = result.list;
        this.dataSource.data = result.list as LatePaymentCharges[];
      } else {
  ...
  openAddPopup(link: any) {
    ...
    // console.log(link);
    const modalRef = this.modalService.open(LatePaymentChargesEntryComponent);
  ...
  open(content: any, AutoId: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      if (result === 'yes') {
        this.latePaymentChargesService.delete(AutoId).subscribe(
          result => {
            // console.log(Countries);
            if (result.isSuccess) {
  ...
  DeleteSelection(content: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      if (result === 'yes') {
        ...
        if (autoIds.length > 0) {
          this.latePaymentChargesService.deleteMultiple(autoIds).subscribe(
            result => {
              if (result.isSuccess) {
                this.alertify.success(result.message);
              } else {
  ...
  ExportData(fileType: string) {
    ...
    if (autoIds.length != -1) {
      let ModuleName: string = 'Late Payment Charges'
      if (fileType == "pdf") {
