...
import { CaseTypePracticeAreaEntryComponent } from '../case-type-practice-area-entry/case-type-practice-area-entry.component';

@Component({
  selector: 'app-case-type-practice-area-list',
  templateUrl: './case-type-practice-area-list.component.html',
  styleUrls: ['./case-type-practice-area-list.component.css']
})
export class CaseTypePracticeAreaListComponent implements OnInit, AfterViewInit {
  public displayedColumns = ['select', 'practiceArea', 'description', 'action'];
  public dataSource = new MatTableDataSource<CaseTypePracticeArea>();
  @ViewChild(MatSort) sort!: MatSort
  @ViewChild(MatPaginator) paginator!: MatPaginator;

  selection = new SelectionModel<CaseTypePracticeArea>(true, []);

  CaseTypePracticeAreas: CaseTypePracticeArea[] = []
  closeResult: string = "";
  constructor(private http: HttpClient, public modalService: NgbModal,
    private caseTypePracticeAreaService: CaseTypePracticeAreaService, private alertify: AlertifyService,
  ...
  ngOnInit() {
    this.caseTypePracticeAreaService.getAll().subscribe(result => {
      ...
      if (result.isSuccess) {
        ...
        this.dataSource.data = result.list as CaseTypePracticeArea[];
      } else {
  ...
  openAddPopup(link: any) {
    ...
    // console.log(link);
    const modalRef = this.modalService.open(CaseTypePracticeAreaEntryComponent);
  ...
  open(content: any, autoId: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      if (result === 'yes') {
        this.caseTypePracticeAreaService.delete(autoId).subscribe(
          result => {
            // console.log(Countries);
            if (result.isSuccess) {
  ...
  DeleteSelection(content: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      if (result === 'yes') {
        ...
        if (autoIds.length > 0) {
          this.caseTypePracticeAreaService.deleteMultiple(autoIds).subscribe(
            result => {
              if (result.isSuccess) {
                this.alertify.success(result.message);
              } else {
  ...
  ExportData(fileType: string) {
    ...
    if (autoIds.length != -1) {
      let ModuleName: string = 'Case Type Practice Area'
      if (fileType == "pdf") {
