...
import { PlanPricingEntryComponent } from '../plan-pricing-entry/plan-pricing-entry.component';

@Component({
  selector: 'app-plan-pricing-list',
  templateUrl: './plan-pricing-list.component.html',
  styleUrls: ['./plan-pricing-list.component.css']
})
export class PlanPricingListComponent implements OnInit {

  public displayedColumns = ['select', 'planName', 'planPercent', 'description', 'sortOrder',
    'defaultPlan', 'action'];
  public dataSource = new MatTableDataSource<PlanPricing>();
  @ViewChild(MatSort) sort!: MatSort
  @ViewChild(MatPaginator) paginator!: MatPaginator;

  selection = new SelectionModel<PlanPricing>(true, []);
  ...
  constructor(private http: HttpClient, public modalService: NgbModal,
    private planPricingService: PlanPricingService, private alertify: AlertifyService,
    private datePipe: DatePipe, private importExportDataService: ImportExportDataService) { }

  ngOnInit(): void {

    this.planPricingService.getAll().subscribe(result => {
      if (result.isSuccess) {
        this.dataSource.data = result.list as PlanPricing[];
      } else {
  ...
  openAddPopup(link: any) {
    ...
    // console.log(link);
    const modalRef = this.modalService.open(PlanPricingEntryComponent);
  ...
  open(content: any, AutoId: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      if (result === 'yes') {
        this.planPricingService.delete(AutoId).subscribe(
          result => {
  ...
  DeleteSelection(content: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      if (result === 'yes') {
        ...
        if (autoIds.length > 0) {
          this.planPricingService.deleteMultiple(autoIds).subscribe(
            result => {
              if (result.isSuccess) {
                this.alertify.success(result.message);
              } else {
  ...
  ExportData(fileType: string) {
    ...
    if (autoIds.length != -1) {
      let ModuleName: string = 'Plan Pricing'
      if (fileType == "pdf") {
