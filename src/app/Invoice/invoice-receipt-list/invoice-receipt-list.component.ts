...
import { AppearanceInvoiceDetails, AppearanceInvoiceReceipt } from 'src/app/models/invoice';
import { AlertifyService } from 'src/app/services/alertify.service';
import { ImportExportDataService } from 'src/app/services/import-export-data.service';
import { InvoiceService } from 'src/app/services/invoice.service';

@Component({
  selector: 'app-invoice-receipt-list',
  templateUrl: './invoice-receipt-list.component.html',
  styleUrls: ['./invoice-receipt-list.component.css']
})
export class InvoiceReceiptListComponent implements OnInit {
  ...
  public displayedColumns = ['select', 'firmName', 'firstName', 'lastName', 'attFirstName', 'attLastName', 'invoiceNo',
    ...
    'videturCommPer', 'videturCommAmount', 'latePayment', 'totalAmount', 'status', 'receiptAmount', 'paymentMode', 'paymentSatus', 'action'];

  public dataSource = new MatTableDataSource<AppearanceInvoiceReceipt>();
  ...
  selection = new SelectionModel<AppearanceInvoiceReceipt>(true, []);
  ...
  ExportData(fileType: string) {
    ...
    if (autoIds.length != -1) {
      let ModuleName: string = 'Appearance Invoice Receipt'
      if (fileType == "pdf") {
  ...
  fillData() {
    this.invoiceService.getInvoiceReceiptAll().subscribe(result => {
      // console.log(Countries);
      if (result.isSuccess) {
        // this.CaseTypePracticeAreas = result.list;
        this.dataSource.data = result.list as AppearanceInvoiceReceipt[];
      } else {
        this.alertify.error(result.message);
      }
    });
